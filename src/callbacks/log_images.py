...
from utils.datamodule import dict_collate_and_pad
import string
    ...
    def __init__(
        ...
        if negative_prompts == "random_prompt":
            self.negative_prompts = torch.from_numpy(
                np.load(Path(root_dir) / Path("src/utils/flan_t5_xl_random.npy"))
            )
        else:
        ...
        if self.log_conditional:
            if self.mode == "class_conditional":
                ...
                self.train_dataset = TextCondPromptBed(
                    Path(self.root_dir) / Path("datasets/text_prompt_testbed"),
                    self.num_samples_per_cond,
